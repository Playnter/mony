var drawerMessages = [
  { id: 1, accent: "red", text: "انتي أجمل صدفة حصلت في حياتي ❤️" },
  ...
  { id: 6, accent: "white", text: "متنسيش بس الحضن و البوسه بتوع كل يوم حبيت افكرك هنا كمان لتكوني بتنسي" },
  { id: 7, accent: "red", text: "تلاجه اهو مش حارمك من حاجه علشان يبقي اسم وفعل ههههههه" },
  { id: 8, accent: "white", text: "مش هتحني بقي ؟ 🙂" },
];
...
function buildFridge() {
  ...
  var drawerSpacing = 0.56;
  var drawerStackOffsetY = 0.08;
  var drawerTopY = ((drawerMessages.length - 1) * drawerSpacing) / 2 + drawerStackOffsetY;

  drawerLayouts = drawerMessages.map(function (message, index) {
    return {
      id: message.id,
      accent: message.accent,
      y: drawerTopY - index * drawerSpacing,
    };
  });
  ...
  drawerLayouts.forEach(function (layout) {
    ...
    var tapZone = new THREE.Mesh(
      new THREE.BoxGeometry(2.26, 0.64, 1.04),
      new THREE.MeshBasicMaterial({ transparent: true, opacity: 0 }),
    );
    tapZone.position.set(0, 0, 1.08);
...
function bindEvents() {
  ...
  gateInput.addEventListener("input", unlockExperience);
...
function unlockExperience() {
  if (gateInput.value.trim().toLowerCase() !== "sqr") {
    return;
  }
...
function onPointerDown(event) {
  ...
  var target = intersects.find(function (intersection) {
    return intersection.object.userData.type === (!state.sequenceStarted ? "door" : "drawer");
  });
  if (!target) {
    return;
  }
